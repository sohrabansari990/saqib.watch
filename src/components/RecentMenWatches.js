...
export default function RecentMenWatches() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            if (cachedCatalog?.products?.length > 0) {
                let menProducts = cachedCatalog.products.filter(
                    (p) => p.category?.toLowerCase() === "men" && p.imageUrl
                );
                menProducts = sortProductsByNewest(menProducts);
            ...
            try {
                ...
                console.error("RecentMenWatches load failed", err);
            } finally {
